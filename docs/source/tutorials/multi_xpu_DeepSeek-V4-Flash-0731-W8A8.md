...
:::{warning}
截至 2026-08-16，这个移植已经通过 8 卡启动、完整权重加载、`/health`、模型枚举
和冷启动后的首个贪心请求，但连续第二个独立请求仍可能退化为 token id 0，
`logprobs` 也可能因非有限浮点数返回 HTTP 400。因此它是可继续调试的实验版本，
还不能作为生产可用结论。最后必须运行本文的连续请求测试，不能只检查首请求。
:::
...
上预计会暴露已知的第二请求问题；只有该测试稳定通过，多轮重复和压力测试也通过，
才能把状态从“启动链路打通”提升为“可部署”。
